...
typedef void(^completeBlock)(NSInteger currentScore);
...
-(instancetype)initWithFrame:(CGRect)frame finish:(finishBlock)finish{
    if (self = [super initWithFrame:frame]) {
        _numberOfStars = 5;
        _rateStyle = WholeStar;
        _complete = ^(NSInteger currentScore){
            finish(currentScore);
        };
        [self createStarView];
    }
...
-(void)createStarView{
    ...
    self.foregroundStarView.frame = CGRectMake(0, 0, self.bounds.size.width*(int)(_currentScore/self.numberOfStars), self.bounds.size.height);
...
- (UIView *)createStarViewWithImage1:(NSString *)imageName {
