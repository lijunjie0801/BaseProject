...
- (instancetype)initWithFrame:(CGRect)frame delegate:(id<SGPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames isdouble:(NSString *)isdouble{
...
+(instancetype)pageTitleViewWithFrame:(CGRect)frame delegate:(id<SGPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames isdouble:(NSString *)isdouble{
    return [[self alloc]initWithFrame:frame delegate:delegate titleNames:titleNames isdouble:isdouble];
}
- (void)initialization {
...
- (void)setupTitleButtons {
    ...
    if (self.allBtnWidth <= self.bounds.size.width) {
       /// SGPageTitleView 不可滚动
        CGFloat btnY = 0;
        CGFloat btnW =0;
        if ([self.isdouble integerValue]==0) {
            btnW  = self.frame.size.width / (self.titleArr.count+1);
        }else{
            btnW  = self.frame.size.width / self.titleArr.count;
        }
       
        CGFloat btnH = self.frame.size.height - SGIndicatorHeight+1;
        CGFloat jianju=(kScreen_Width-self.titleArr.count*btnW)/(self.titleArr.count-1);
        for (NSInteger index = 0; index < self.titleArr.count; index++) {
            UIButton *btn = [UIButton buttonWithType:(UIButtonTypeCustom)];
            // 设置 frame
            CGFloat btnX =0;
             if ([self.isdouble integerValue]==0) {
                  btnX = (btnW+jianju) * index;
             }else{
                 btnX = btnW * index;
             }
...
-(void)setIsdouble:(NSString *)isdouble{
        _isdouble=isdouble;
}
